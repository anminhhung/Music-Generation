...
def load_training_data():
    with open(os.path.join(pwd, "data", "irish.abc"), "r") as f:
        text = f.read()
...
def abc2wav(abc_file):
    path_to_tool = os.path.join(pwd, 'bin', 'abc2wav')
...
def play_generate_song(generated_text):
    songs = extract_song_snippet(generated_text)
    if len(songs) == 0:
        print("No valid songs found in generated text. Try training\
            model longer or increasing the amount of generated music\
            to ensure complete songs are generated!")
